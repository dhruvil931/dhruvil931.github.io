          ...
          <div className={styles.cardList}>
            {education.map((edu, index) => (
              <div className={styles.card} key={index}>
                ...
                <h4 className={styles.institution}>{edu.institution} | {edu.location}</h4>
                <p className={styles.coursework}><strong>Coursework:</strong> {edu.coursework}</p>
              </div>
            ))}
          </div>
        </div>

        <div ref={actReveal} className={`reveal ${styles.sectionBlock}`} style={{ marginTop: '5rem' }}>
